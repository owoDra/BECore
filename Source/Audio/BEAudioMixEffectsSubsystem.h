...
#include "BEAudioMixEffectsSubsystem.generated.h"
...
USTRUCT()
struct FBEAudioSubmixEffectsChain
{
	GENERATED_BODY()
 ...
 */
UCLASS()
class BECORE_API UBEAudioMixEffectsSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	// Default Sound Control Bus Mix retrieved from the BE Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> DefaultBaseMix = nullptr;

	// Loading Screen Sound Control Bus Mix retrieved from the BE Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> LoadingScreenMix = nullptr;

	// User Sound Control Bus Mix retrieved from the BE Audio Settings
	UPROPERTY(Transient)
	...
	UPROPERTY(Transient)
	TArray<FBEAudioSubmixEffectsChain> HDRSubmixEffectChain;

	// Submix Effect hain Overrides to apply when HDR Audio is turned off
	UPROPERTY(Transient)
	TArray<FBEAudioSubmixEffectsChain> LDRSubmixEffectChain;
