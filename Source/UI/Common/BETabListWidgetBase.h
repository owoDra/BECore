...
#include "BETabListWidgetBase.generated.h"

class UCommonButtonBase;

USTRUCT(BlueprintType)
struct FBETabDescriptor
{
	GENERATED_BODY()
...
class UBETabButtonInterface : public UInterface
{
	GENERATED_BODY()
};

class IBETabButtonInterface
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintNativeEvent, Category = "Tab Button")
	void SetTabLabelInfo(const FBETabDescriptor& TabDescriptor);
};

UCLASS(Blueprintable, BlueprintType, Abstract, meta = (DisableNativeTick))
class BECORE_API UBETabListWidgetBase : public UCommonTabListWidgetBase
{
	GENERATED_BODY()
	
public:
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Tab List")
	bool GetPreregisteredTabInfo(const FName TabNameId, FBETabDescriptor& OutTabInfo);

	/** Helper method to get at all the preregistered tab infos */
	const TArray<FBETabDescriptor>& GetAllPreregisteredTabInfos() { return PreregisteredTabInfoArray; }
	...
	UFUNCTION(BlueprintCallable, Category = "Tab List")
	bool RegisterDynamicTab(const FBETabDescriptor& TabDescriptor);
	...
	DECLARE_EVENT_TwoParams(UBETabListWidgetBase, FOnTabContentCreatedNative, FName /* TabId */, UCommonUserWidget* /* TabWidget */);
...
private:
	...
	TArray<FBETabDescriptor> PreregisteredTabInfoArray;
	 ...
	 */
	UPROPERTY()
	TMap<FName, FBETabDescriptor> PendingTabLabelInfoMap;
};
