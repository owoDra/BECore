...
void UBETabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void UBETabListWidgetBase::NativeConstruct()
{
...
void UBETabListWidgetBase::NativeDestruct()
{
	for (FBETabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UBETabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FBETabDescriptor& OutTabInfo)
{
	const FBETabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FBETabDescriptor& TabInfo) -> bool
	{
...
void UBETabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FBETabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UBETabListWidgetBase::RegisterDynamicTab(const FBETabDescriptor& TabDescriptor)
{
...
void UBETabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FBETabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UBETabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void UBETabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FBETabDescriptor* TabInfoPtr = nullptr;
	
	FBETabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(UBETabButtonInterface::StaticClass()))
	{
		...
		{
			IBETabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool UBETabListWidgetBase::IsFirstTabActive() const
{
...
bool UBETabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool UBETabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 UBETabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void UBETabListWidgetBase::SetupTabs()
{
	for (FBETabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
