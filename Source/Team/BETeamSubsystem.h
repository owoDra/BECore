...
#include "BETeamSubsystem.generated.h"

class AActor;
class ABEPlayerState;
class ABETeamInfoBase;
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnBETeamDisplayAssetChangedDelegate, const UBETeamDisplayAsset*, DisplayAsset);


USTRUCT()
struct FBETeamTrackingInfo
{
	GENERATED_BODY()

public:
	UPROPERTY()
	TObjectPtr<ABETeamPublicInfo> PublicInfo = nullptr;

	UPROPERTY()
	TObjectPtr<ABETeamPrivateInfo> PrivateInfo = nullptr;

	UPROPERTY()
	TObjectPtr<UBETeamDisplayAsset> DisplayAsset = nullptr;

	UPROPERTY()
	FOnBETeamDisplayAssetChangedDelegate OnTeamDisplayAssetChanged;

public:
	void SetTeamInfo(ABETeamInfoBase* Info);
	void RemoveTeamInfo(ABETeamInfoBase* Info);
};

// Result of comparing the team affiliation for two actors
UENUM(BlueprintType)
enum class EBETeamComparison : uint8
{
...
UCLASS()
class BECORE_API UBETeamSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
public:
	...
	void RegisterTeamInfo(ABETeamInfoBase* TeamInfo);
	void UnregisterTeamInfo(ABETeamInfoBase* TeamInfo);
	...
	const ABEPlayerState* FindPlayerStateFromActor(const AActor* PossibleTeamActor) const;
	...
	EBETeamComparison CompareTeams(const UObject* A, const UObject* B, int32& TeamIdA, int32& TeamIdB) const;

	// Compare the teams of two actors and returns a value indicating if they are on same teams, different teams, or one/both are invalid
	EBETeamComparison CompareTeams(const UObject* A, const UObject* B) const;
	...
	UFUNCTION(BlueprintCallable, Category=Teams)
	UBETeamDisplayAsset* GetTeamDisplayAsset(int32 TeamId, int32 ViewerTeamId);

	// Gets the team display asset for the specified team, from the perspective of the specified team
	// (You have to specify a viewer too, in case the game mode is in a 'local player is always blue team' sort of situation)
	UFUNCTION(BlueprintCallable, Category = Teams)
	UBETeamDisplayAsset* GetEffectiveTeamDisplayAsset(int32 TeamId, UObject* ViewerTeamAgent);
	...
	// Gets the list of teams
	UFUNCTION(BlueprintCallable, BlueprintPure = false, Category = Teams)
		TArray<int32> GetEnemyTeamIDsFromActor(const UObject* TestActor) const;

	// Called when a team display asset has been edited, causes all team color observers to update
	void NotifyTeamDisplayAssetModified(UBETeamDisplayAsset* ModifiedAsset);

	// Register for a team display asset notification for the specified team ID
	FOnBETeamDisplayAssetChangedDelegate& GetTeamDisplayAssetChangedDelegate(int32 TeamId);

private:
	UPROPERTY()
	TMap<int32, FBETeamTrackingInfo> TeamMap;
