...
#include "BEDevelopmentStatics.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BEDevelopmentStatics)


bool UBEDevelopmentStatics::ShouldSkipDirectlyToGameplay()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<UBEDeveloperSettings>()->bTestFullGameFlowInPIE;
	}
#endif
	return false;
}

bool UBEDevelopmentStatics::ShouldLoadCosmeticBackgrounds()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<UBEDeveloperSettings>()->bSkipLoadingCosmeticBackgroundsInPIE;
	}
#endif
	return true;
}

bool UBEDevelopmentStatics::CanPlayerBotsAttack()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return GetDefault<UBEDeveloperSettings>()->bAllowPlayerBotsToAttack;
	}
#endif
	return true;
}
...
UWorld* UBEDevelopmentStatics::FindPlayInEditorAuthorityWorld()
{
	check(GEngine);
...
TArray<FAssetData> UBEDevelopmentStatics::GetAllBlueprints()
{
...
UClass* UBEDevelopmentStatics::FindBlueprintClass(const FString& TargetNameRaw, UClass* DesiredBaseClass)
{
	FString TargetName = TargetNameRaw;
	TargetName.RemoveFromEnd(TEXT("_C"), ESearchCase::CaseSensitive);

	TArray<FAssetData> BlueprintList = UBEDevelopmentStatics::GetAllBlueprints();
...
UClass* UBEDevelopmentStatics::FindClassByShortName(const FString& SearchToken, UClass* DesiredBaseClass, bool bLogFailures)
{
