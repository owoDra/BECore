...
#include "BEPlatformEmulationSettings.generated.h"

/**
 * Platform emulation settings
 */
UCLASS(config = EditorPerProjectUserSettings, MinimalAPI)
class UBEPlatformEmulationSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UBEPlatformEmulationSettings();
...
private:
	...
	UPROPERTY(EditAnywhere, config, Category = PlatformEmulation, meta = (ConsoleVariable = "BE.Settings.ApplyFrameRateSettingsInPIE"))
	...
	UPROPERTY(EditAnywhere, config, Category = PlatformEmulation, meta = (ConsoleVariable = "BE.Settings.ApplyFrontEndPerformanceOptionsInPIE"))
		bool bApplyFrontEndPerformanceOptionsInPIE = false;

	// Should we apply experience/platform emulated device profiles in PIE?
	UPROPERTY(EditAnywhere, config, Category = PlatformEmulation, meta = (InlineEditConditionToggle, ConsoleVariable = "BE.Settings.ApplyDeviceProfilesInPIE"))
