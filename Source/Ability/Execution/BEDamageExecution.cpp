...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BEDamageExecution)
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UBECombatSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UBEDamageExecution::UBEDamageExecution()
{
	RelevantAttributesToCapture.Add(DamageStatics().BaseDamageDef);
}

void UBEDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	FBEGameplayEffectContext* TypedContext = FBEGameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	if (HitActor)
	{
		UBETeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UBETeamSubsystem>();
	...
	if (const IBEAbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	{
		if (const UPhysicalMaterial* PhysMat = TypedContext->GetPhysicalMaterial())
		{
			PhysicalMaterialAttenuation = AbilitySource->GetPhysicalMaterialAttenuation(Distance, PhysMat, SourceTags, TargetTags);
		}
	}
	PhysicalMaterialAttenuation = FMath::Max(PhysicalMaterialAttenuation, 0.0f);
	...
	if (DamageDone > 0.0f)
	{
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UBEHealthSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
