...
#include "BEPlayerSpawningManagerComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BEPlayerSpawningManagerComponent)

DEFINE_LOG_CATEGORY_STATIC(LogPlayerSpawning, Log, All);


UBEPlayerSpawningManagerComponent::UBEPlayerSpawningManagerComponent(const FObjectInitializer& ObjectInitializer)
...
void UBEPlayerSpawningManagerComponent::InitializeComponent()
{
...
void UBEPlayerSpawningManagerComponent::OnLevelAdded(ULevel* InLevel, UWorld* InWorld)
{
	if (InWorld == GetWorld())
	{
...
void UBEPlayerSpawningManagerComponent::HandleOnActorSpawned(AActor* SpawnedActor)
{
...
AActor* UBEPlayerSpawningManagerComponent::ChoosePlayerStart(AController* Player)
{
	if (Player)
	{
#if WITH_EDITOR
...
#if WITH_EDITOR
APlayerStart* UBEPlayerSpawningManagerComponent::FindPlayFromHereStart(AController* Player)
{
...
bool UBEPlayerSpawningManagerComponent::ControllerCanRestart(AController* Player)
{
...
void UBEPlayerSpawningManagerComponent::FinishRestartPlayer(AController* NewPlayer, const FRotator& StartRotation)
{
...
void UBEPlayerSpawningManagerComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);
}

APlayerStart* UBEPlayerSpawningManagerComponent::GetFirstRandomUnoccupiedPlayerStart(AController* Controller, const TArray<ABEPlayerStart*>& StartPoints) const
{
	if (Controller)
	{
